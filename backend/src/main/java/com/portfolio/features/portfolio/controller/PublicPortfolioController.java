...
@GetMapping("/portfolios/{slug}") public ApiResponse<PublicPortfolioResponse> get(@PathVariable String slug) { var p = portfolios.publicBySlug(slug); var profile = assets.findByPortfolioIdAndAssetType(p.getId(), "PROFILE_IMAGE").map(a -> storage.getPublicUrl(a.getObjectKey())).orElse(PublicAssetUrl.relative(p.getProfileImageUrl())); var resume = assets.findByPortfolioIdAndAssetType(p.getId(), "RESUME").map(a -> new PublicPortfolioResponse.Resume(true, storage.getPublicUrl(a.getObjectKey()))).orElse(new PublicPortfolioResponse.Resume(false, null)); var projectResponses = projects.findByPortfolioIdOrderBySortOrderAscTitleAsc(p.getId()).stream().map(x -> new ProjectResponse(x.getId(), x.getTitle(), x.getSlug(), x.getShortDescription(), x.getDescription(), assets.findByProjectIdAndAssetType(x.getId(), "PROJECT_IMAGE").map(a -> storage.getPublicUrl(a.getObjectKey())).orElse(PublicAssetUrl.relative(x.getThumbnailUrl())), x.getGithubUrl(), x.getDemoUrl(), x.isFeatured(), x.getSortOrder(), x.getTechnologies().stream().map(t -> t.getTechnology()).toList())).toList(); return ApiResponse.success("Portfolio retrieved successfully.", new PublicPortfolioResponse(p.getId(), p.getSlug(), p.getFullName(), p.getHeadline(), p.getBio(), p.getLocation(), p.getPublicEmail(), profile, resume, p.getTemplateKey(), p.getThemeConfig(), design.responses(sectionRepository.findByPortfolioIdOrderByPositionAsc(p.getId()), p.getTemplateKey()), experiences.findByPortfolioIdOrderBySortOrderAscStartDateDesc(p.getId()).stream().map(experienceMapper::toResponse).toList(), educations.findByPortfolioIdOrderBySortOrderAscStartDateDesc(p.getId()).stream().map(educationMapper::toResponse).toList(), skills.findByPortfolioIdOrderBySortOrderAscNameAsc(p.getId()).stream().map(skillMapper::toResponse).toList(), projectResponses, socialLinks.findByPortfolioIdOrderBySortOrderAsc(p.getId()).stream().map(socialMapper::toResponse).toList())); }
}
